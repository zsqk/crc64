...
interface AdaptedImports {
  env: {
    abort(message: number, fileName: number, lineNumber: number, columnNumber: number): void;
...
async function instantiate(module: WebAssembly.Module, imports: ImportObject = {}): Promise<AdaptedExports> {
  ...
  const { exports } = await WebAssembly.instantiate(module, adaptedImports as unknown as WebAssembly.Imports);
  const wasmExports = exports as unknown as WasmExports;
  const memory = wasmExports.memory || (imports.env as Record<string, WebAssembly.ExportValue>)?.memory as WebAssembly.Memory;
